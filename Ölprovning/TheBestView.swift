...
//  Ölprovning
//
//  Created by Petter Gustafsson on 2023-10-31.
//

/*import SwiftUI

struct TheBestView: View {
                ...
                VStack {
                    ScrollView {
                        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 3), spacing: 20) {
                            ForEach(sortedBeers, id: \.self){ beer in
    ...
    var sortedBeers: [Beer] {
            // Sort your beers by score
            return (beerType.beers?.allObjects as? [Beer] ?? []).sorted(by: { $0.score > $1.score })
        }
